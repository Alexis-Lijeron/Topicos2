...
def crear_pdf_para_cliente(nombre, correo, productos, nombre_archivo):
    html = f"""
    ...
    for p in productos:
        ...
        if p.get("categoria"):
            html += f"""
            <div class="categoria"><strong>Categoría:</strong> {p['categoria']}</div>
            """
...
def main():
    ...
    for (chat_id, nombre, correo), productos in agrupado.items():
        archivo = os.path.join(RUTA_SALIDA, f"{nombre.replace(' ', '_')}_{chat_id}.pdf")
        crear_pdf_para_cliente(nombre, correo, productos, archivo)
